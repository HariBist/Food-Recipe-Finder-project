...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    for (const favMealId of favMealList) {
        try {
            ...
            if (favMealId !== null && favMealId !== undefined && favMealId !== 'undefined') {
...
const getMealType = (mealId) => {
    // Check if mealId is null or undefined
    if (mealId === null || mealId === undefined || mealId === 'undefined') {
        console.error('Invalid meal ID:', mealId);
        return null; // Return null for invalid meal ID
    }

    // Trim any leading/trailing spaces from the mealId
    const trimmedMealId = mealId.trim();

    // Logic to determine the meal type based on your IDs
    // You might want to add a prefix or use a different approach
    // For example, if IDs starting with "meal" are meals, "cocktail" for cocktails, etc.
    if (trimmedMealId.startsWith('meal')) {
        ...
        console.error('Invalid meal ID:', trimmedMealId);
        return null; // Return null for invalid meal ID
    }
};
